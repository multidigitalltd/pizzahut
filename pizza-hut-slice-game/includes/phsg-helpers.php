...
if ( ! function_exists( 'phsg_branches' ) ) {
	/**
	 * רשימת סניפי פיצה האט (נכון למועד הבנייה). מקור: pizzahut.co.il/branches.
	 *
	 * @return array
	 */
...
if ( ! function_exists( 'phsg_render_branches' ) ) {
	/**
	 * רינדור רשימת הסניפים ל-מודל (חלון) הסניפים.
	 *
	...
	function phsg_render_branches( $branches_url ) {
		$rows = '';
		foreach ( phsg_branches() as $name ) {
			$rows .= '<li class="phsg-branch">📍 ' . esc_html( $name ) . '</li>';
		}
		$more = '';
		if ( $branches_url ) {
			$more = '<a class="phsg-branches__link" href="' . esc_url( $branches_url ) . '" target="_blank" rel="noopener">' . esc_html__( 'לרשימת הסניפים המלאה והכתובות ‹', 'pizza-hut-slice-game' ) . '</a>';
		}
		return '<ul class="phsg-branches__list">' . $rows . '</ul>' . $more;
	}
}
